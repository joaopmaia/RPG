...
export default function AnimalEditar() {
  ...
  const handleSubmit = (e) => {
    ...
    update('animais', id, payload)
      .then(() => navigate(`/animais/${id}/ficha`))
      .catch((e) => setError(e.message))
  ...
  return (
    <div style={{ maxWidth: '640px' }}>
      <div style={{ marginBottom: '1rem' }}>
        <Link to="/animais">← Lista</Link>
        {' · '}
        <Link to={`/animais/${id}/ficha`}>Ver ficha</Link>
      </div>
      <form onSubmit={handleSubmit} className="card">
        ...
        <div className="form-row">
          <label>Tipo (Terrestre, Aquático, Voador)</label>
          <input type="text" value={doc.tipo || ''} onChange={(e) => handleChange('tipo', e.target.value)} />
        </div>
        <div className="form-row">
          <label>Nível (comum, grande, arcano)</label>
          <input type="text" value={doc.nível ?? ''} onChange={(e) => handleChange('nível', e.target.value)} placeholder="comum" />
        </div>
        <div className="form-row">
          <label>Raça</label>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
          <div className="form-row">
            ...
            <input type="number" min={0} value={doc.arcana_total ?? ''} onChange={(e) => handleChange('arcana_total', e.target.value === '' ? '' : Number(e.target.value))} />
          </div>
          <div className="form-row">
            <label>Arcana atual</label>
            <input type="number" min={0} value={doc.arcana_atual ?? ''} onChange={(e) => handleChange('arcana_atual', e.target.value === '' ? '' : Number(e.target.value))} />
          </div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
          <div className="form-row">
        ...
        <div className="form-row">
          ...
          <input type="text" value={doc.ataque_especial || ''} onChange={(e) => handleChange('ataque_especial', e.target.value)} />
        </div>
        <div className="form-row">
          <label>Efeito ataque crítico</label>
          <input type="text" value={doc.efeito_ataque_critico || ''} onChange={(e) => handleChange('efeito_ataque_critico', e.target.value)} />
        </div>
        <div className="form-row">
          <label>Runas (uma por linha)</label>
