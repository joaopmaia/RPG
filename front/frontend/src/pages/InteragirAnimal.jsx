...
export default function InteragirAnimal() {
  ...
  const temArcana = (parseFloat(doc?.arcana_total) || 0) > 0
  ...
  const aplicarRecuperarArcana = () => {
    ...
    const arcanaAtual = parseFloat(doc.arcana_atual) ?? 0
    const novoArcana = arcanaAtual + v
    update(COLLECTION, id, { arcana_atual: novoArcana })
      .then(() => {
        setDoc((p) => (p ? { ...p, arcana_atual: novoArcana } : null))
        setResumoAcao({ tipo: 'arcana', texto: `${doc.nome} recuperou ${v} de arcana. Arcana atual: ${novoArcana}.` })
      })
      .catch((e) => setError(e.message))
  }
  ...
  return (
    <div style={{ maxWidth: '800px' }}>
      ...
      <div className="card" style={{ marginBottom: '1rem', border: estaMorto ? '2px solid #dc2626' : undefined }}>
        ...
        {estaMorto && (
          <p style={{ color: '#dc2626', fontWeight: 600, marginBottom: '0.5rem' }}>⚠ Este animal está morto (HP = 0).</p>
        )}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '0.75rem' }}>
          ...
          <div><strong>Tipo</strong><br />{doc.tipo ?? '—'}</div>
        ...
        <div className="card" style={{ marginBottom: '1rem' }}>
          ...
          {lootOpen && (
            <ul style={{ margin: 0, paddingLeft: '1.25rem' }}>
              {doc.loot.map((item, i) => (
                <li key={i} style={{ marginBottom: '0.25rem' }}>{typeof item === 'string' ? item : item.nome || JSON.stringify(item)}</li>
              ))}
            </ul>
          )}
        </div>
      ...
      {/* Modais: Curar, Arcana, Gastar Arcana, Dano, Dano Direto, Rolar Atributo, Alterar Nome */}
      ...
      {temArcana && (
        <>
          <Modal open={modalArcana} onClose={fecharModalComResumo} title={`Recuperar arcana — ${doc.nome}`}>
            {resumoAcao?.tipo === 'arcana' ? (
              ...
              <>
                <p style={{ fontSize: '0.9rem', color: 'var(--parchment-dark)' }}>Valor será somado à arcana atual.</p>
                <div className="form-row">
                  <label>Valor</label>
          ...
          <Modal open={modalGastarArcana} onClose={() => { setModalGastarArcana(false); setGastarArcanaVal(''); }} title={`Gastar arcana — ${doc.nome}`}>
